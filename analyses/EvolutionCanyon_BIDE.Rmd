...
```{r}
dir <- "../microbide/SbyS/Tests"
input <- list.dirs(dir, full.names = T)
names <- list.dirs(dir, full.names = F)
...
# Conditions
cond1 <- list.files(dir, pattern = "Condition1.txt", recursive = T, full.names = T)
cond6 <- list.files(dir, pattern = "Condition6.txt", recursive = T, full.names = T)
```
...
pair.dis <- function(x){
  ...
  pairs <- as.matrix(bide.dist)[seq(1, 80, by = 2), seq(2, 80, by = 2)]
  pair.dist <- diag(pairs)
  pair.dist.slope <- gsub("[0-9]*_.*", "", row.names(pairs))
...
for(i in 1:length(cond1)){
  pair.distances1[i, 4:7] <- pair.dis(cond1[i])
}

# Condition 6
pair.distances6 <- as.data.frame(matrix(NA, nrow = length(cond6), ncol = 7))
colnames(pair.distances6) <- c("lgp", "im", "dk", "north.mean", "north.sem", 
                              "south.mean", "south.sem")
pair.distances6$lgp <- lgp; pair.distances6$im <- im; pair.distances6$dk <- dk

for(i in 1:length(cond6)){
  pair.distances6[i, 4:7] <- pair.dis(cond6[i])
}
```
...
# Reorganize Condition 6 for Plotting
pair.dis.b <- data.frame(
  lgp = rep(pair.distances6$lgp, 2),
  im = rep(pair.distances6$im, 2),
  dk = rep(pair.distances6$dk, 2),
  dist = c(pair.distances6$north.mean, pair.distances6$south.mean), 
  col = as.character(c(rep("sienna", length(pair.distances6$north.mean)), 
          rep("forestgreen", length(pair.distances6$south.mean)))),
  stringsAsFactors = F)
pair.dis.b <- pair.dis.b[sample(1:dim(pair.dis.b)[1]),]
...
axis(side = 2, labels = T, las = 1, cex.axis = 1.25)
mtext("Pair Dissimilarity", side = 2, line = 3.5, outer = F)
mtext("Condition 6 (The new 4)", side = 2, line = 5.5, outer = F)
mtext("Lognormal Parameter", side = 1, line = 3, outer = F)
box(lwd = 1.5)
...
for(i in 1:length(cond1)){
  site.distances1[i, 4:7] <- site.dis(cond1[i])
}

# Condition 6
site.distances6 <- as.data.frame(matrix(NA, nrow = length(cond6), ncol = 7))
colnames(site.distances6) <- c("lgp", "im", "dk", "north", "south", 
                              "all", "active")
site.distances6$lgp <- lgp; site.distances6$im <- im; site.distances6$dk <- dk

for(i in 1:length(cond6)){
  site.distances6[i, 4:7] <- site.dis(cond6[i])
}
```
...
plot(site.distances1$all ~ site.distances1$lgp, col = "gray", pch = 16, 
     ylim = c(0, 0.2), las = 1, cex = 0.75,
     xlab = "Lognormal Parameter", ylab = "Site Dissimilarity",
     main = "Condition 1")

plot(site.distances1$all ~ site.distances1$im, col = "gray", pch = 16, 
     ylim = c(0, 0.2), las = 1, cex = 0.75,
     xlab = "Lognormal Parameter", ylab = "Site Distances, All (BC)",
     main = "Condition 1")

plot(site.distances1$all ~ log(site.distances1$dk), col = "gray", pch = 16, 
     ylim = c(0, 0.2), las = 1, cex = 0.75,
     xlab = "Lognormal Parameter", ylab = "Site Distances, All (BC)",
     main = "Condition 1")

plot(site.distances6$all ~ site.distances6$lgp, col = "gray", pch = 16, 
     ylim = c(0, 0.2), las = 1, cex = 0.75,
     xlab = "Lognormal Parameter", ylab = "Site Dissimilarity")

plot(site.distances6$all ~ site.distances6$im, col = "gray", pch = 16, 
     ylim = c(0, 0.2), las = 1, cex = 0.75,
     xlab = "Lognormal Parameter", ylab = "Site Distances, All (BC)")

plot(site.distances6$all ~ log(site.distances6$dk), col = "gray", pch = 16, 
     ylim = c(0, 0.2), las = 1, cex = 0.75,
     xlab = "Lognormal Parameter", ylab = "Site Distances, All (BC)")
