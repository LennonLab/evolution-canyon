...
input <- list.files("../microbide/SbyS/20140805/", "*.txt")
...
for (i in 1:length(input)){
  dbRDA.output[[input[i]]] = ec.dbRDA.sim(shared = paste("../microbide/SbyS/20140805/", 
                                             input[i], sep=""), 
                                             level = "0.03", 
...
for (p in 1:length(dbRDA.output)){
  ...
  # Initiate Plot
  plot(RDA$CAP2, RDA$CAP1, xlab = "", ylab="",
  ...
  points(RDA$CAP2, RDA$CAP1, pch=mol.shape, cex=2.0, col="black", bg=slope.color, lwd=2)
  box(lwd=2)
  
  text(x = -0.38, y = 0.34, labs[p], cex = 1.5)
}
