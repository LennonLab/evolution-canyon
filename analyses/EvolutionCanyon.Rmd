...
# Figure 1: Evolution Canyon and Simulated Landscape
Photo of Evolution Canyon and corresponding landscape used in the simulations

```{r fig.width=2, fig.height=4,echo=FALSE, fig.cap="Figure 1: Evolution Canyon"}
...
Ordination of simulated data capturing processes/scenarios 
```{r}
# Define Analysis
ec.dbRDA.sim <- function(shared = " ", level = "0.03", design = " "){
  ...
  RDA <- as.data.frame(vegan::scores(dbRDA, display = c("wa"), 
                                     scaling = "sites"))
  RDA$molecule <- as.factor(design$molecule)
  RDA$slope <- as.factor(design$slope)
  RDA$labs <- as.factor(slope.molecule.concat)
  
  # Permanova
  adonis.out <- adonis2(dataREL.l ~ RDA$molecule * RDA$slope)
  
  # RDA Stats
  rda.stats <- anova(dbRDA, by="terms", permu=999)
  var.part <- varpart(vegdist(dataREL.l, method = "bray"),  
                      ~ slope, ~ molecule)
...
for (i in 1:length(input)){
  dbRDA.output[[input[i]]] = ec.dbRDA.sim(
    shared = paste(dir, input[i], sep=""),
    level = "0.03", design = "../data/simmy.design.txt")
  }

```

## Figure 2: Ordination from BIDE models
```{r}
# Plot Parameters
par(opar)
par(mfrow=c(1,1), mar=c(0.5,0.5,0,0), oma=c(3.5, 3.5, 0.5, 0.5))
...
for (p in 1:length(dbRDA.output)){
  ...
  plot(PCoA$points[, 1], PCoA$points[, 2], xlab = "", ylab="",
    xlim=x.dim,ylim= y.dim, pch=16, cex=2, type="n",xaxt="n",
    yaxt="n", cex.lab=1.5, cex.axis=1)

  axis(side=1, las=1, label=x.lab[p], lwd.ticks = 2, cex.axis = 1)
  axis(side=2, las=0, label=y.lab[p], lwd.ticks = 2, cex.axis = 1)
  axis(side = 1, lwd.ticks = 2, tck=0.01, labels = F,  cex.axis = 1.25) 
  axis(side = 2, lwd.ticks = 2, tck=0.01, labels = F,  cex.axis = 1.25)
  axis(side = 3, lwd.ticks = 2, tck=0.01, labels = F,  cex.axis = 1.25)     
  axis(side = 4, lwd.ticks = 2, tck=0.01, labels = F,  cex.axis = 1.25)
  ...
  slope.color <- rep(NA, dim(RDA)[1])
    for (i in 1:length(slope.color)){
      if (RDA$slope[i] == levels(RDA$slope)[1]) {slope.color[i] = "green3"}
      else {slope.color[i] = "wheat3"}
    }
  ...
  points(PCoA$points[, 1][ran.ord], PCoA$points[, 2][ran.ord], 
         ...
         cex=1.5, col="black", bg=slope.color[ran.ord], lwd=1.5)
  box(lwd=2)
  
  text(x = -0.09, y = 0.075, labs[p], cex = 1.5)
  text(x = 0, y = -0.075, adj = 0.5, 
       labels = paste("PCoA 1 = ", explainvar1, " %", sep = ""), cex = 1)
  text(x = -0.09, y = 0, adj = 0.5, srt=90,
       labels = paste("PCoA 2 = ", explainvar2, " %", sep = ""), cex = 1)
}
  
mtext("Axis 1", side = 1, cex = 1, outer=T, line = 2)
mtext("Axis 2", side = 2, cex = 1, outer=T, line = 2) 
  
# Legend
par(mar=c(0, 0, 0, 0))
plot.new()
legend("center", c("Active; Mesic    ", "Active; Xeric    ", 
                   "All; Mesic  ", "All; Xeric  "), 
      pt.lwd=2, col="black", pt.bg=c("green3", "wheat3", 
                                             "green3", "wheat3"),
      pch=c(21,21,22,22), bty='n', ncol=4, cex=1, pt.cex=1.5)  

# Copy plot to PNG and PDF Files
# dev.copy(png, file = "../figures/BIDE_Ordination.png", 
#          width = 6, height = 4)

dev.copy2pdf(file = "../figures/BIDE_Ordination.pdf",
...
for(i in 1:length(input)){
  ...
  colnames(alpha.temp) <- c("Condition", "Sample", "Rep", 
                            "Slope", "Group", 
                            "N", "S", "E")
...
# Plot Parameters
par(opar)
par(mfrow = c(1, 1), mar = c(5, 4, 1, 1), oma = c(1, 1, 0, 0))
...
axis(side = 1, las = 1, 
     ...
     labels = rep(c("All", "Active"), 4), font = 1, las = 2)
axis(side=1, las=1, at = c(3, 10, 18, 25), line = 3.25, 
     labels=c("Cond. 1", "Cond. 2", "Cond. 3", "Cond. 4"), 
     tick = F, font = 2)
axis(side=2, las=1, label=T, lwd.ticks = 1.5, cex = 1)

legend("bottomleft", legend = c("Xeric", "Mesic"), pch = 22, 
...
ec_data2 <- ec_data[-c(which(rownames(ec_data) %in% 
                               c(names(low.coverage)))), ]
...
## Figure 3: Ordination of Evolution Canyon
```{r}
# Plot Parameters
par(opar)
...
text(-0.16, 0.21, "Xeric Slope")
text(0.16, 0.21, "Mesic Slope")

text(0.28, -0.1, "Total", srt = 90, cex = 1.1)
...
# Copy plot to PNG and PDF Files
# dev.copy(png, file = "../figures/EC_Ordination.png", 
#          width = 3, height = 3)

dev.copy2pdf(file = "../figures/EC_Ordination.pdf",
    width = 6, height = 4, bg = "white", compress = F, out.type = "pdf")
...
# PERMANOVA
Adonis.b <- adonis2(dataREL.l ~ design_red$molecule * design_red$slope)
Adonis.b
```
...
```{r}
# PCoA
ec.bc <- vegdist(dataPA, method = "bray", binary = T)
ec.pcoa <- cmdscale(ec.bc, k = 3, eig = T)
...
PCoA <- data.frame(Dim1 = ec.pcoa$points[, 1],
                      Dim2 = ec.pcoa$points[, 2])
PCoA$molecule <- molecule
PCoA$slope <- slope
PCoA$station <- station
PCoA$labs <- slope.molecule.concat
...
```{r}
# Data Checks
rownames(ec_data_red)
rownames(design_red)
all.equal(rownames(ec_data_red), rownames(design_red))
...
# Are there differences in coverage based on the two
...
# Plot Parameters
par(opar)
par(mfrow = c(1, 1), mar = c(4, 4, 1, 2), oma = c(1, 2, 0, 0))
...
# Indicator Taxa
```{r}
# Import Taxonomy and Fill in Blanks
ec.tax <- read.tax(taxonomy = "../data/EC.bac.final.0.03.taxonomy", 
                   format = "silva")

ec.tax.un <- read.tax(taxonomy = "../data/EC.bac.final.unidentified.taxonomy", 
                      format = "silva")

ec.tax[which(ec.tax$OTU %in% ec.tax.un$OTU), ] <- ec.tax.un[1:dim(ec.tax.un)[1], ]
...
# Treatment code: slope.molecule.concat



# a) Run phi analysis
phi.ind <- multipatt(dataREL, cluster =  slope.molecule.concat, 
                     func = "r.g", control = how(nperm = 999))
# Subset for significant correlations greater than 0.7
phi.sig.ind <- phi.ind$sign[ which(phi.ind$sign[7] <= 0.01 & phi.ind$sign[6] >= abs(0.7)), ]

phi.sig.ind.tax <- phi.sig.ind[, 1:4]


str(summary(phi.ind, indvalcomp=TRUE, alpha = 0.01, minstat = 0.7))


png(filename = "../figures/EC_Euler.png", 
    width = 1600, height = 1000, res = 96 * 2)


plot(euler(phi.sig.ind.tax, shape = "ellipse"), 
     quantities = TRUE,  
     labels = c("Xeric DNA", "Xeric RNA", 
                "Mesic DNA", "Mesic RNA"),
     fills = c("wheat3", "burlywood3", "green3", 
                       "darkolivegreen3"))
...
## Show Plot
img <- readPNG("../figures/EC_Euler.png")
grid.raster(img)
...
```{r}
# Mexic DNA
dim(phi.sig.ind.tax[which(phi.sig.ind.tax$s.EUDNA == 1 & rowSums(phi.sig.ind.tax) == 1), ])

dim(phi.sig.ind.tax[which(phi.sig.ind.tax$s.AFDNA == 1 & rowSums(phi.sig.ind.tax) == 1), ])

dim(phi.sig.ind.tax[which(phi.sig.ind.tax$s.AFRNA == 1 & rowSums(phi.sig.ind.tax) == 1), ])

dim(phi.sig.ind.tax[which(phi.sig.ind.tax$s.EURNA == 1 & rowSums(phi.sig.ind.tax) == 1), ])
...
# i.) Taxa associated with Mesic DNA 
phi.sig.A <- phi.sig.ind[ which(phi.sig.ind[1] == 1),] 
imp.otu.A <- as.vector(rownames(phi.sig.A))
imp.tax.A <- ec.tax[which (ec.tax$OTU %in% imp.otu.A), ] 
imp.tax.A.table <- table(imp.tax.A$Phylum)
imp.tax.A.table.per <- imp.tax.A.table/dim(imp.tax.A)[1]*100
# Acidobacteria 8.62%, Actinobacteria 25%, Plactomycetes 25% Proteobacteria 12%) Taxa associated with Xeric DNA Samples 

phi.sig.B <- phi.sig.ind[ which(phi.sig.ind[2] == 1),] 
imp.otu.B <- as.vector(rownames(phi.sig.B))
imp.tax.B <- ec.tax[which (ec.tax$OTU %in% imp.otu.B), ] 
imp.tax.B.table <- table(imp.tax.B$Phylum)
imp.tax.B.table.per <- imp.tax.A.table/dim(imp.tax.B)[1]*100
